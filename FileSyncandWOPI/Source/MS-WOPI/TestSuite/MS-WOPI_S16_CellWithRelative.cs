namespace Microsoft.Protocols.TestSuites.MS_WOPI
{
    ...
    /// <summary>
    /// A class which contains test cases used to capture the requirements related target with CellSubRequest operation.
    /// </summary>
    [TestClass]
    public class MS_WOPI_S16_CellWithRelative : S01_Cell
    {
        #region Variables
        ...
        public static void MSWOPISharedTestClassInitialize(TestContext testContext)
        {
            ...
            if (!TestSuiteHelper.VerifyRunInSupportProducts(MS_WOPI_S16_CellWithRelative.BaseTestSite))
            {
                return;
            }

            TestSuiteHelper.InitializeTestSuite(MS_WOPI_S16_CellWithRelative.BaseTestSite);
        ...
        protected override void InitializeContext(string requestFileUrl, string userName, string password, string domain)
        {
            ...
            TestSuiteHelper.InitializeContextForShare(requestFileUrl, userName, password, domain, CellStoreOperationType.RealativeModified, this.Site);
        }
