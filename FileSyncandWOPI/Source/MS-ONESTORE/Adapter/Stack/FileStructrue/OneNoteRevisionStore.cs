namespace Microsoft.Protocols.TestSuites.MS_ONESTORE
{
    ...
    public class OneNoteRevisionStoreFile
    {
        ...
        public void DoDeserializeFromByteArray(byte[] byteArray)
        {
            ...
            FileChunkReference64x32 fileNodeListRef = this.Header.fcrFileNodeListRoot;
            if (fileNodeListRef.IsfcrNil() == false && fileNodeListRef.IsfcrZero() == false)
            {
                do
                {
                    FileNodeListFragment fileNodeList = new FileNodeListFragment(fileNodeListRef.Cb);
                    fileNodeList.DoDeserializeFromByteArray(byteArray, (int)fileNodeListRef.Stp);
                    this.FileNodeList.Add(fileNodeList);
                    fileNodeListRef = fileNodeList.nextFragment;
                }
                while (fileNodeListRef.IsfcrNil() == false);
            }
