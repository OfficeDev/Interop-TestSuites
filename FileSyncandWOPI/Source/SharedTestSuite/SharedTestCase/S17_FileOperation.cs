            ...
            if (SharedContext.Current.IsMsFsshttpRequirementsCaptured)
            {
                ...
                if (Common.IsRequirementEnabled("MS-FSSHTTP-FSSHTTPB", 11268, this.Site))
                {
                    ...
                    Site.CaptureRequirementIfAreEqual<GenericErrorCodeTypes>(
                        ...
                        "MS-FSSHTTP",
                        11268,
                        @"[In Appendix B: Product Behavior]  If the specified attributes[FileOperation] are not provided, the implementation does return error code. &lt;33&gt; Section 2.3.1.33:  SharePoint Server 2016 return ""HighLevelExceptionThrown"" error code as part of the SubResponseData element associated with the file operation subresponse(Microsoft SharePoint Server 2016 follow this behavior).");
                }
            }
