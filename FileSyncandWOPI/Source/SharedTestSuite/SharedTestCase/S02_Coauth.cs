            ...
            if (SharedContext.Current.IsMsFsshttpRequirementsCaptured)
            {
                ...
                if (Common.IsRequirementEnabled("MS-FSSHTTP-FSSHTTPB", 3104, this.Site))
                {
                    Site.CaptureRequirementIfAreEqual<ErrorCodeType>(
                             ...
                             "MS-FSSHTTP",
                             3104,
                             @"[In Appendix B: Product Behavior] When the coauthoring feature is disabled on the protocol server, if the AllowFallbackToExclusive attribute is set to false, the implementation does return an error code value set to ""FileNotLockedOnServerAsCoauthDisabled"". (<42> Section 3.1.4.3.1:  SharePoint Server 2010 will return an error code value ""FileNotLockedOnServerAsCoauthDisabled"",SharePoint Server 2019 and SharePoint Server Subscription Edition will return an error code value ""LockNotConvertedAsCoauthDisabled"", if the AllowFallbackToExclusive attribute is set to false.)");
                ...
                // Verify MS-FSSHTTP requirement: MS-FSSHTTP_R310401 and MS-FSSHTTP_R382
            ...
            if (SharedContext.Current.IsMsFsshttpRequirementsCaptured)
            {
                if (Common.IsRequirementEnabled("MS-FSSHTTP-FSSHTTPB", 3108, this.Site))
                {
                    ...
                    Site.CaptureRequirementIfAreEqual<ErrorCodeType>(
                             ...
                             "MS-FSSHTTP",
                             3108,
                             @"[In Appendix B: Product Behavior] When the refresh of the shared lock on the file for that specific client fails because the file is no longer locked since the timeout value expired on the lock in the file coauthoring tracker, if the coauthoring feature is disabled, the implementation does return an error code value set to ""FileNotLockedOnServerAsCoauthDisabled"". (<45> Section 3.1.4.3.3:  SharePoint Server 2010 will return an error code value ""FileNotLockedOnServerAsCoauthDisabled"", SharePoint Server 2019 and SharePoint Server Subscription Edition will return an error code value ""Success"".)");
                }
