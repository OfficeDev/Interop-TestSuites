            ...
            if (SharedContext.Current.IsMsFsshttpRequirementsCaptured)
            {
                ...
                if (Common.IsRequirementEnabled("MS-FSSHTTP-FSSHTTPB", 3118, this.Site))
                {
                    ...
                    Site.CaptureRequirementIfAreEqual<ErrorCodeType>(
                             ...
                             "MS-FSSHTTP",
                             3118,
                             @"[In Appendix B: Product Behavior] When the coauthoring feature is disabled on the protocol server, if the AllowFallbackToExclusive attribute is set to false when sending a Get Lock subrequest, the implementation does return an error code value set to ""FileNotLockedOnServerAsCoauthDisabled"". <49> Section 3.1.4.4.1:  SharePoint Server 2010 will return an error code value ""FileNotLockedOnServerAsCoauthDisabled"", SharePoint Server 2019 and SharePoint Server Subscription Edition will return an error code value ""LockNotConvertedAsCoauthDisabled"", if the AllowFallbackToExclusive attribute is set to false.)");
                }
            ...
            if (SharedContext.Current.IsMsFsshttpRequirementsCaptured)
            {
                ...
                if (Common.IsRequirementEnabled("MS-FSSHTTP-FSSHTTPB", 3121, this.Site))
                {
                    Site.CaptureRequirementIfAreEqual<ErrorCodeType>(
                             ...
                             "MS-FSSHTTP",
                             3121,
                             @"[In Appendix B: Product Behavior] When the refresh of the shared lock on the file for that specific client fails because the file is no longer locked since the timeout value expired on the lock, if the coauthoring feature is disabled and if the AllowFallbackToExclusive attribute is set to false, the implementation does return an error code value set to ""FileNotLockedOnServerAsCoauthDisabled"". (<52> Section 3.1.4.4.3:  SharePoint Server 2010 will return an error code value ""FileNotLockedOnServerAsCoauthDisabled"",SharePoint Server 2019 and SharePoint Server Subscription Edition will return an error code value ""Success"".)");
                }
