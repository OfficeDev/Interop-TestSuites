...
$enableMailboxFileName = "InServer-EnableMailbox" + "-" + $PtfPropUserName + "-" + $timestamp + ".ps1"
...
Copy-Item  "$FolderPath\InServer-EnableMailbox.ps1" -Destination "$ServerScriptUncPath\$enableMailboxFileName" -Force
Copy-Item  "$FolderPath\AddExchangeSnapIn.ps1" -Destination "$ServerScriptUncPath\$addExchangeSnapInFileName" -Force

net use $share /d /y

#--------------------------------------------------------------------------------------------------------------------------------------
# This script is used to call a script in server to enable a user's mailbox
#--------------------------------------------------------------------------------------------------------------------------------------
...
param(
	...
	[String]$enableMailboxFileName,			# Indicate the ps file to enable the mailbox
...
#----------------------------------------------------------------------------
# Remotely run a script in server to enable mailbox
#----------------------------------------------------------------------------
$psFile = "$ServerScriptPath\$enableMailboxFileName"
...
try
{
	Remove-Item -Path "$ServerScriptPath\$enableMailboxFileName"
...
return $result
}-ArgumentList $userName,$serverName,$serverVersion,$ServerScriptPath,$credentialUserName,$PtfPropUserPassword,$enableMailboxFileName,$addExchangeSnapInFileName