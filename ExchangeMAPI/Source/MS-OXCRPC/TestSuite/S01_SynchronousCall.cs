    ...
    public class S01_SynchronousCall : TestSuiteBase
    {
        ...
        public void MSOXCRPC_S01_TC08_TestROPReadStream()
        {
            ...
            Site.CaptureRequirementIfIsTrue(
                ...
                972,
                @"[In rgbOut Output Buffer] The server MUST NOT return more data to the client than the client originally requested.");
        ...
        public void MSOXCRPC_S01_TC20_TestRopWriteStreamExtended()
        {
            ...
            Site.Assert.AreEqual<uint>(0, this.returnValue, "Call EcDoConnectEx should succeed and '0' is expected to be returned. The returned value is {0}.", this.returnValue);
            bool isSupportPackWriteStream = false;
            foreach (AUX_SERVER_TOPOLOGY_STRUCTURE rgbAuxValue in rgbAuxOutValue)
            {
                if (rgbAuxValue.Header.Version == 0x01 && rgbAuxValue.Header.Type == 0x46)
                {
                    isSupportPackWriteStream = (BitConverter.ToInt32(rgbAuxValue.Payload, 0) & 0x00000002) == 0x00000002;
                }
            }
            #endregion

            if (isSupportPackWriteStream)
            {
                #region Logon to mailbox
                this.rgbIn = AdapterHelper.ComposeRgbIn(ROPCommandType.RopLogon, this.unusedInfo, this.userPrivilege);
                this.pcbOut = ConstValues.ValidpcbOut;
        ...
        public void MSOXCRPC_S01_TC21_TestRopFastTransferDestinationPutBuffer()
        {
            ...
            Site.Assert.AreEqual<uint>(0, this.returnValue, "Call EcDoConnectEx should succeed and '0' is expected to be returned. The returned value is {0}.", this.returnValue);
            bool isSupportPackFastTransfer = false;
            foreach (AUX_SERVER_TOPOLOGY_STRUCTURE rgbAuxValue in rgbAuxOutValue)
            {
                if (rgbAuxValue.Header.Version == 0x01 && rgbAuxValue.Header.Type == 0x46)
                {
                    isSupportPackFastTransfer = (BitConverter.ToInt32(rgbAuxValue.Payload, 0) & 0x00000001) == 0x00000001;
                }
            }
            #endregion
            if (isSupportPackFastTransfer)
            {
                #region Logon to mailbox
                this.rgbIn = AdapterHelper.ComposeRgbIn(ROPCommandType.RopLogon, this.unusedInfo, this.userPrivilege);
                this.pcbOut = ConstValues.ValidpcbOut;
